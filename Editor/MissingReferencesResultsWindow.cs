...
namespace Editor
{
    ...
    public class MissingReferencesResultsWindow : EditorWindow
    {
        ...
        private readonly Dictionary<GameObject, int> missingComponents = new();
        private readonly Dictionary<GameObject, IList<ComponentProperty>> missingReferences = new();
        ...
        private static bool ValidateOneGameObjectSelected()
        {
            return (Selection.transforms?.Length ?? 0) == 1;
        }

        private void OnGUI()
        {
            SelectedGameObject =
                (GameObject)EditorGUILayout.ObjectField("Game Object: ", SelectedGameObject, typeof(GameObject), true);
            ...
            showMissingComponents =
                drawCollapsibleSection("Missing Components", missingComponents.Count, showMissingComponents);
            ...
            showMissingReferences = drawCollapsibleSection("Missing References",
                missingReferences.Values.Sum(list => list.Count),
        ...
        private void refreshMissingReferences()
        {
            ...
            if (SelectedGameObject)
                MissingReferencesFinder.FindMissingReferences(SelectedGameObject,
                    true,
                    missingComponents,
                    missingReferences);
